...
3> async function in node js to get value
...
Node js , Application of Nodejs, Advantages of Nodejs and disadvantage , Why is Nodejs Single Threaded,Package,Library
...
nodejs event loop event listener 

Where does NodeJs can be used?
 What is Asynchronuos, How we handle the asynchronous function, why is nodejs multi threaded.
what is NodeJS, what is iframe,  git commands , MySQL quries,event in JavaScript,  what is promises .
...
6. Let's say we were to build a Facebook event application ? 
...
Node Question: create a server , child threads, event emitter,  file system
...
what is NPM, diff between frontend and backend,why node is popular, what is event emiter.
...
How to store data in database without using array,
What is Database?
7. How cache storage is differnt from database.
0.can you use multiple collections in database?
.what kind of data and the what you store in your database?
)Which database you people have used in your projects.
...
diff between relational and non-relational database
Which database do you are familiar with
Diff between nosql and sql
which database is prefered in Node ,
 insert data in database(query),
 which database you use, how do you connect your project to database.
...
What nosql data base?Why we use is it?
...
How to connect two documents in mongodb, what is populate,
What is Indexing, what is queries in mongodb , what is mongoose,
 why mongodb over SQL,
- What is BSON?
- What is MongoDB document store format ? what is the option that creates a document in mongodb if we want to find a doc and it didnt exist ? what is the use of populate?
Is mongodb schemaless
. Let's say we were to build a Facebook event application ? What database you would use: MySQL or MongoDB ? Reasons ?
...
role of mongodb, how to update data in mongo db)
...
- If you are sending a response from 
 ...
 what is aggregation in mongodb, upsert in mongo query,
.given a table in a row id and price you have to add all the price using a mongodb query and you have to write that query

mongodb connection
What if mongodb not connect
Resolve that error
Set and Get on mongodb
What is aggregation and give some name? 10. What unwind do in mongoDB?Tell me two error you get from mongoDB during project implementation 
...
2. What is cloud Providers, 3. Services of AWS ,4.difference between Aws (s3) and Database,
...
Explain aws s3 and in which file you kept it. 3. Explain the whole code flow. 4. Where did you use aws s3 in your project and explain the whole flow. why we use aws?
2. What is cloud Providers, 3. Services of AWS ,4.difference between Aws (s3) and Database,
 Aws s3
Where used aws s3 
 aws bucket concept
...
user login api, jwt token related questions
...
How backend deals with jwt and frontend deals with Jwt .
...
HTML Elements-- check the correct opening and closing
...
how can we call html in JavaScript,
20)input types in html 
21) if I want background color red the how can I make it
22)border in css
1 depth question on css ?
Give name any 5 input types in Html?
5)html tags
can we update date in input tag in html, what other option can we use for creating buttons, how to make form in html, how to mask password in html,
...
###  Discussion about the tech. stack I have, Why we need DBMS, Some frameworks of CSS. 


### 5. Have used any css frameworks ?
. Css Property - What is padding, write property for padding from right and left.
CSS flex,
...
11.If you want to collect data from two collection at a time is it possible? If possible then how & which query you use?
 ...
 explain url shortner project ,Why use bcrypt in e commerce project,   
...
=> project discussion (shopping cart, , situational questions like for certain scenario what would you do, how do you handle such sittuation(these were easy and small question, which we have already implemented in our project),
...
Asked about projects Wanted to know our projects in deep and Grilled Specifically About  Project 5th Shopping Cart
-Roles in projects
...
Run the Project, How many projects you did by yourself, 
...
. how to create a new nodejs project ( only commands)
...
3. Backend -  http methods, requests, header, few output finding question 
...
Mysql join, types of join in mySql ,
...
4. Sql query problem
8. What LIKE do in SQL ,4. how to connect two table in sql
...
1. Identify correct Dom(dsa)

**Questions:**
# 1. wildCard=> match the string with the given symbol, 

```js
input:"++++$*{2} adep7pp" ,output: true
...
# 11. Compare the following examples, build logic and solve in any programming language(use offline or online IDE).
 ```js
 //e.g 1)
 input : ABDEF
 output : GFECB
 ...
 output : EUY
 ```
# 12. With a single integer as the input, generate the following until a = x [series of numbers as shown in below examples]
```js
 // e.g. 1) input : 1
...
# Wildcards :- Have the function Wildcards(str) read str which will contain two strings separated by a space. 
...
# Question  to valid parenthese

# wildcard matching level hard. Two string given one in incoded form other its decripted version you have return if it matches or not.
1. reverse each word in sentence
2. queue using stack explanation (no code)
3. swap two elements in linkedlist
4. a puzzle(no code)
5.  linked list -> what is the length of the linked list , delete a node and swap 2 nodes.
6. Queque & Stack->implement queue using stack ,how many stacks are required to implement queue
7. Que on pipeline (Operating System)
...
# coding question - for a given string we need to print a desired output
```js
i/p: {ab{ac}a} 
o/p:
-aba*
--ac
```
for repeatition of character we need to add * with the repeated character ,we need to maintain order of parent and child (could also be in child to parent order, but we need to maintain order) ,there can be n number of childs and n number of nested childs
...
let arr = [20,-4,10,5,-2,-1],
 output : Expected res = [-4,20,-2,10,-1,5]

 ```
# coding question remove duplicates from 2 lists (from themselves and from eachother)
```js
let  l1 = [1,3,2,6,6] , l2 = [1,2,7,8] ,
output - l1 = [1,3,2,6] , l2 =[7,8]
```
...
*b.* in a dark room, there are 10 red socks, 10 blue socks and 10 green socks.How much less quantity you can bring once so that you'll get one pair of socks

# There are 2 strings of same length, check each character of both the strings, and print '0' if first string's character is smaller than other and '1' otherwise
```js
...
2- DSA question largest number in array, largest product of any two nums in array. 
...
Logical questions 
DSA=> different type of logical question related to fibonacci series, Merge Sort, ForEach , MAP, Reduce , OAuth ,  Express, try catch, given assignment, 
...
5.Name of dataStructures and explain each of them in one line, 
...
js data types
One DSA problem , About indexing, About transation ,  Who is webpage render
...
Q. asked me to sort an array live coding.
...
Valid parenthisis,anagram ,frequency counter
...
DSA - print the small number of range which the number lies,
  ...
  input: aaaaddddggggg output:4a4d5g write the code.
...
MCQ, bit count of given decimal value, fibonacci series like 1,2,3,4,..... where 1 maps to 'a', 2 maps to 'b', respectivily so on, if 26 maps to 'z' and 27 is maps to 'a' respectively so on, what is abstraction, what is class component, what is not linear data structure, tree traversal, and 1- What is array?
...
And one question is also from string.find the string whose length is n and sum is S.
1. Merge sort
2. Fibonacci
